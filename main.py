...
def Selecionar_TbPonto():
    conexao = conecta_bd()
    cursor = conexao.cursor(dictionary=True)
    comando = f"select cdPonto, cdAcessoIntelbras, dsCardNo, dsCardName, DATE_FORMAT(STR_TO_DATE(dsRegistro01, '%Y-%m-%d %H:%i:%s'), '%d/%m/%Y') AS data, DATE_FORMAT(STR_TO_DATE(dsRegistro01, '%Y-%m-%d %H:%i:%s'), '%H:%i') AS dsRegistro01, DATE_FORMAT(STR_TO_DATE(dsRegistro02, '%Y-%m-%d %H:%i:%s'), '%H:%i') AS dsRegistro02, DATE_FORMAT(STR_TO_DATE(dsRegistro03, '%Y-%m-%d %H:%i:%s'), '%H:%i') AS dsRegistro03, DATE_FORMAT(STR_TO_DATE(dsRegistro04, '%Y-%m-%d %H:%i:%s'), '%H:%i') AS dsRegistro04, DATE_FORMAT(STR_TO_DATE(dsRegistro05, '%Y-%m-%d %H:%i:%s'), '%H:%i') AS dsRegistro05, DATE_FORMAT(STR_TO_DATE(dsRegistro06, '%Y-%m-%d %H:%i:%s'), '%H:%i') AS dsRegistro06, dtRegistro  from DbIntelliMetrics.TbPonto;"
    cursor.execute(comando)
    resultado = cursor.fetchall()
    cursor.close()
    conexao.close()
